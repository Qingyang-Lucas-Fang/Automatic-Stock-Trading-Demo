...
def update_dashboard(n):
    ...
    if not os.path.exists("trade_log.csv") or not os.path.exists("strategy_config.json"):
        return go.Figure(), go.Figure(), "INITIALIZING ENGINE: WAITING FOR DATA..."

    try:
        # 2. Read Logs (Limit to last 500 points for performance)
        df = pd.read_csv("trade_log.csv").tail(500)
        if df.empty:
        ...
        pnl_color = "#00ff88" if current_eq >= 100 else "#ff4d4d"
