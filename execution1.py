...
def execution_engine():
    ...
    while True:
        try:
            ...
            if conf['name'] == "MFI":
                _, pos = optimizer1.run_mfi(df, conf['p1'], conf['p2'])
            elif conf['name'] == "MR":
                _, pos = optimizer1.run_mean_reversion(df, conf['p1'], conf['p2'])
            else:
                _, pos = optimizer1.run_rsi_breakout(df, conf['p1'], conf['p2'])

            # 4. Update Equity (Simplified)
            if last_close is not None:
                returns = (current_price - last_close) / last_close
                equity *= (1 + (returns * pos))
